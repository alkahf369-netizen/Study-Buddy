const MCQCard = ({ q, index, selected, onSelect }) => {
  ...
  return (
    ...
    >
      {/* Faint ruled lines â€” mimics exam paper */}
      <div
        aria-hidden="true"
      ...
      />
      {/* Warm inner glow on the left edge â€” like a paper fold shadow */}
      <div
        aria-hidden="true"
      ...
      <div className="relative grid gap-2 sm:grid-cols-2">
        {q.options.map((opt, i) => {
          ...
          // Tick-box states:
          //   - unanswered             â†’ empty box
          //   - user picked & correct  â†’ black tick
          //   - user picked & wrong    â†’ red cross
          //   - user picked wrong AND this is the right one â†’ auto black tick
...
const GRADE_TABLE = [
  ...
  { min: 93, grade: "A",  remark: "Excellent â€” very well done." },
  ...
  { min: 80, grade: "B-", remark: "Good attempt â€” a bit more to polish." },
  { min: 77, grade: "C+", remark: "Decent, keep practicing." },
  { min: 73, grade: "C",  remark: "Room to improve â€” you can do it." },
  { min: 70, grade: "C-", remark: "Needs more review." },
  { min: 67, grade: "D+", remark: "Revise the basics and try again." },
  { min: 63, grade: "D",  remark: "Keep at it â€” review and retry." },
  { min: 60, grade: "D-", remark: "Don't give up â€” practice helps." },
 ...
 * per-user leaderboard later â€” the public shape is preserved.
 */
    ...
    >
      ...
      {compare?.status === "new-best" && (
        ...
        >
          ...
          >
            <span style={{ fontSize: "11px", letterSpacing: "0.1em" }}>â˜… NEW â˜…</span>
            ...
            >
              {compare.status === "first" && (
                <span style={{ transform: "rotate(-0.5deg)", display: "inline-block" }}>
                  First attempt â€” setting your benchmark.
                </span>
              )}
              {compare.status === "new-best" && (
                <span style={{ transform: "rotate(-0.5deg)", display: "inline-block" }}>
                  New best! <span style={{ fontWeight: 700 }}>+{compare.delta}</span> from your last best Â· Attempt #{compare.attempts}
                </span>
              )}
              {compare.status === "matched" && (
                <span style={{ transform: "rotate(-0.5deg)", display: "inline-block" }}>
                  = Matched your best Â· Attempt #{compare.attempts}
                </span>
              )}
              {compare.status === "below" && (
                <span style={{ transform: "rotate(-0.5deg)", display: "inline-block" }}>
                  Prev best: {compare.priorBest.bestCorrect}/{total} ({compare.priorBest.bestGrade}) Â· Attempt #{compare.attempts}
                </span>
              )}
            </div>
          )}
...
const QuizView = ({ questions, answers, onAnswer, onReset, onRetry, title }) => {
  ...
  // Result popup state â€” auto-opens the moment the quiz transitions to complete.
  ...
  return (
    <section className="mx-auto w-full max-w-3xl px-4 pb-32 sm:px-6">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            {total} questions Â· Pick an option to reveal the explanation
          </p>
        </div>
        <button
      ...
      {/* Floating "View Result" pill â€” visible after completion if the popup was dismissed */}
